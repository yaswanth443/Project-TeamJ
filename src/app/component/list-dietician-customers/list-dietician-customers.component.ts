...
@Component({
  selector: 'app-list-dietician-customers',
  templateUrl: './list-dietician-customers.component.html',
  styleUrls: ['./list-dietician-customers.component.scss']
})
export class ListDieticianCustomersComponent implements OnInit {
  ...
  loadNewCustomersData() {
    this.dieticianService.get(AppConstants.GET_DIETITIAN_NEW_CUSTOMERS).pipe(
      take(1)
    ).subscribe({
      next: (res) => {
